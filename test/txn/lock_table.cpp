...
TEST(LockTableTest, BasicTest) {
  ...
  TxnId txn1 = 0;
  EXPECT_TRUE(table.Lock(sk1.as_ref(), txn1).ok());
  EXPECT_TRUE(table.Lock(sk2.as_ref(), txn1).ok());
  TxnId txn2 = 1;
  auto future = util::LaunchAsync([&]() {
    util::Timer timer;
    EXPECT_TRUE(table.Lock(sk1.as_ref(), txn2).ok());
    EXPECT_GT(timer.GetElapsed(), 5 * util::MillSec);
  });
  bthread_usleep(10 * util::MillSec);
  EXPECT_TRUE(table.Unlock(sk1.as_ref(), txn1).ok());
  EXPECT_TRUE(table.Unlock(sk2.as_ref(), txn1).ok());
  future->Wait();
}

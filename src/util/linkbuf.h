 ...
 * @brief Linkbuf port from innodb
 * @version 0.1
 * @date 2023-01-25
 *
...
// TODO(sheep): implement more specific one
...
*/
template <typename Position = uint64_t> class Link_buf {
public:
...
private:
  ...
  alignas(ARCANEDB_CACHE_LINE_SIZE) std::atomic<Position> m_tail;
};
...
template <typename Position> Link_buf<Position>::Link_buf() : Link_buf(0) {}
...
template <typename Position> Link_buf<Position>::~Link_buf() { free(); }

template <typename Position> void Link_buf<Position>::free() {
  if (m_links != nullptr) {
    delete[] m_links;
    m_links = nullptr;
  }
...
bool Link_buf<Position>::advance_tail_until(Stop_condition stop_condition,
  ...
  CHECK(from == m_tail.load(std::memory_order_acquire));
...
template <typename Position> inline bool Link_buf<Position>::advance_tail() {
...
template <typename Position> inline Position Link_buf<Position>::tail() const {
...
void Link_buf<Position>::validate_no_links(Position begin, Position end) {
  ...
  for (; begin < end; ++begin) {
    ...
    CHECK(slot.load() <= tail);
  }
}

template <typename Position> void Link_buf<Position>::validate_no_links() {

...
#if defined(__GLIBC__) || defined(__GLIBCXX__) || defined(__GNU_LIBRARY__) ||  \
    defined(__ANDROID__)
#include <endian.h>
#else
...
constexpr uint16_t swap_u16(uint16_t n) noexcept { return (n << 8) | (n >> 8); }
...
template <typename T, typename> struct is_input_iterator : std::false_type {};
...
template <size_t N> struct integral_type_for {
  ...
  using type = typename std::conditional<
      ...
      typename std::conditional<
          (N > sizeof(uint16_t)),
          typename std::conditional<(N > sizeof(uint32_t)), uint64_t,
                                    uint32_t>::type,
          uint16_t>::type,
      uint8_t>::type;
};
...
/**
 * Parses `sizeof(T)` bytes from the memory pointed to by `it`, and reconstructs
 * from it an integer of type `T`, converting from the specified `Order` to host
 * byte order.
 *
...
template <class T, class InputIt> MND_CONSTEXPR T read_le(InputIt it) noexcept {
  return read<order::little, T>(it);
}

template <class T, class InputIt> MND_CONSTEXPR T read_be(InputIt it) noexcept {
...
template <order Order, class T, class InputIt>
MND_CONSTEXPR T parse(InputIt it) noexcept {
  return read<Order, T>(it);
}

/**
 * Writes each byte of `h` to the memory pointed to by `it`, such that it
 * converts the byte order of `h` from host byte order to the specified `Order`.
 *
 * The value type of the iterator must represent a byte, that is:
 * `sizeof(typename std::iterator_traits<InputIt>::value_type) == sizeof(char)`.
 *
 ...
 * first, and among the data to be written are integers. E.g.:
 * ```
 * std::array<char, 1024> buffer;
...
/**
 * Writes the first `N` byte of `h` to the memory pointed to by `it`, such that
 * it converts the byte order of `h` from host byte order to the specified
 * `Order`. Thus `N` must be at most `sizeof(h)`.
 *
 * The value type of the iterator must represent a byte, that is:
 * `sizeof(typename std::iterator_traits<InputIt>::value_type) == sizeof(char)`.
 *
 ...
 */
template <class T> MND_CONSTEXPR T reverse(const T &t);
 ...
 * ```
 */
template <class T> MND_CONSTEXPR T host_to_network(const T &t);
 ...
 * ```
 */
template <class T> MND_CONSTEXPR T network_to_host(const T &t);
...
read(InputIt it) noexcept {
  static_assert(sizeof(T) >= MaxNBytes,
                "Can only read at most sizeof(T) bytes");
  T h = 0;
...
write(const T &h, OutputIt it) noexcept {
  static_assert(sizeof(T) >= MaxNBytes,
                "Can only write at most sizeof(T) bytes");
  for (int shift = 8 * (int(MaxNBytes) - 1); shift >= 0; shift -= 8) {
    *it++ = static_cast<uint8_t>((h >> shift) & 0xff);
  }
}

/** Converts `h` to LITTLE endian order, writing it to buffer pointed to by
 * `it`. */
template <order Order, class T, class OutputIt, size_t MaxNBytes = sizeof(T)>
...
template <size_t Size> struct byte_swapper {};

template <> struct byte_swapper<2> {
  template <class T> T operator()(const T &t) { return MND_BYTE_SWAP_16(t); }
};

template <> struct byte_swapper<4> {
  template <class T> T operator()(const T &t) { return MND_BYTE_SWAP_32(t); }
};

template <> struct byte_swapper<8> {
  template <class T> T operator()(const T &t) { return MND_BYTE_SWAP_64(t); }
};

// --

#ifndef MND_UNKNOWN_ENDIANNESS
template <order Order> struct conditional_reverser {
  template <class T> MND_CONSTEXPR T operator()(const T &t) {
    return reverse(t);
  }
};

template <> struct conditional_reverser<order::host> {
  template <class T> MND_CONSTEXPR T operator()(const T &t) { return t; }
};
#endif // MND_UNKNOWN_ENDIANNESS
...
template <class T> MND_CONSTEXPR T network_to_host(const T &t) {
  // hton and ntoh are essentially the same, as they both do a byte swap if and
  // only if the host's and network's byte orders differ.

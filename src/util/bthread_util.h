...
class BthreadFuture : public BthreadFutureBase {
...
template <>
class BthreadFuture<void> : public BthreadFutureBase {
public:
    BthreadFuture() = default;
    void Get() {
        Wait();
...
std::shared_ptr<BthreadFuture<ReturnType>> LaunchAsync(Func &&func) noexcept {
    static_assert(std::is_same_v<ReturnType, typename FunctionTask<Func>::ReturnType>);
    auto *task = FunctionTask<Func>::New(std::forward<decltype(func)>(func));
    auto future = task->GetFuture();
    bthread_attr_t attr = BTHREAD_ATTR_NORMAL;
    bthread_t id;
    int ret = bthread_start_background(&id, &attr, FunctionTask<Func>::BthreadWrapper, task);
    assert(ret == 0);
    return future;
}

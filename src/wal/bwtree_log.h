 ...
 * @brief
 * Format:
 * | type 1 byte | txn_id 8 byte | write ts 4 byte | row |
 */
...
public:
  ...
  void SetRow(TxnId txn_id, TxnTs write_ts, const property::Row &row) noexcept {
  ...
  void DeleteRow(TxnId txn_id, TxnTs write_ts,
  ...
  void SetTs(TxnId txn_id, TxnTs commit_ts,
             property::SortKeysRef sort_key) noexcept {
    auto offset = writer_.Offset();
